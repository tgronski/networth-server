...
describe("Goals Endpoints", function () {
  let db;
  const { testUsers, testGoals } = helpers.makeWalletsFixtures();
  ...
  before("clean the table", () => db.raw("TRUNCATE networth_goals CASCADE"));

  afterEach("cleanup", () => db.raw("TRUNCATE networth_goals CASCADE"));
